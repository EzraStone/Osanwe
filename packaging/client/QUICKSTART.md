...
Osanwe runs as a local app. The public website never receives the relay secret, free-test
invitation, provider key, prompt, answer, or local history.
...
4. Enter the relay secret in the masked native prompt. It is held only for that app session and is
   not written to disk or put on a command line.
5. Osanwe opens in a dedicated local app window. For the invited free beta, open **Settings → Free
   test access**, choose the separate invitation JSON file, and activate it. The local app stores the
   invitation and unspent blind-signed tokens in your Windows user data; it never receives or saves
   the gateway's Groq key. Bring-your-own provider keys remain session-only in Settings.
...
4. Paste the relay secret when asked. It is passed through the process environment, removed before
   browser helpers start, and is not saved in the config file or placed on the command line. In the
   local app, open **Settings → Free test access** and choose the separate invitation JSON file.
...
the enrollment and browser-only history. The free-test wallet is kept under
`%APPDATA%\Osanwe\trial-wallet.db`. Delete saved history from Settings first, then delete both
folders if you want to remove all local Osanwe data.
